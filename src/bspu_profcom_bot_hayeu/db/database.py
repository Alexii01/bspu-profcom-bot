...
from bspu_profcom_bot_hayeu.db.admin import Admin
...
def _connect() -> aiosqlite.Connection:
    conn = aiosqlite.connect(constants.DatabasePath)
    conn.row_factory = aiosqlite.Row
    return conn


async def __create_tables_if_not_present():
    await _connect().executescript(f"""
...
async def __super_maintainer_exists():
    return Admin.pull_by_flags(constants.AdminFlags.IS_SUPER | constants.AdminFlags.IS_MAINTAINER)
...
async def __create_super_maintainer_if_not_present():
    ...
    # Create super admin
    [admin, password] = await Admin.new(public_name=None, name_base="Новый админ ", flags=None)
