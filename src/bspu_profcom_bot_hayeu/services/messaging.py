...
#         if self.chat_data.last_message.text == msg_text and self.chat_data.last_keyboard == kbd:
#             logger.warning(f"Updating message to be the same. text: '{text}', keyboard: {kbd}")
#             return

#         if self.chat_data.last_keyboard and (type(self.chat_data.last_keyboard) is not type(kbd)):
#             self.clear_keyboard()
...
#         tmp = await self.last_message.reply_text(text="...", reply_markup=ReplyKeyboardRemove())
#         self.last_keyboard = None
#         self.last_keyboard_name = None
#         await tmp.delete()
...
# update_or_send_msg
#
# delete_messages(Iterable[Message])
# delete_all_messages()
# drop_all_messages()
# clear_keyboard
# ~~~~~~~~~~~~~~~~~~~
...
async def update_msg(
    context: BspuContext, text_alias: str | None, keyboard_alias: str | None, *args, **kwargs
):
    pass
